...
import { getAllGiftCards, redeemGiftCard } from "@/api/giftCards";
...
const EventDetails = () => {
  ...
  const { mutate: handleRedeem, isPending: redeeming } = useMutation({
    mutationFn: async () => {
      console.log(" Starting redeem process...");
      const allCards = await getAllGiftCards();
      console.log("🎁 All Gift Cards:", allCards);

      const activeCard = allCards.find(
        (g: any) =>
          g.status === "active" &&
          (g.couple?._id === event.user._id || g.couple?._id === event.user)
      );
      console.log(" Active card found:", activeCard);

      if (!activeCard) throw new Error("No active gift card found");
      ...
      return redeemGiftCard(activeCard._id, {
        redeemedBy: event.user._id || "",
      });
    },
    onSuccess: ({ updatedBudget, cardAmount }) => {
    ...
    onError: () => {
      Toast.show({
        type: "error",
        text1: "Failed to redeem gift card ❌",
        visibilityTime: 4000,
      });
    },
  });
...
const styles = StyleSheet.create({
  ...
  progressFill: { height: 8, borderRadius: 4, backgroundColor: "green" },
  overBudget: { color: colors.danger, fontWeight: "700" },
  underBudget: { color: "green", fontWeight: "700" },
  redeemBtn: {
