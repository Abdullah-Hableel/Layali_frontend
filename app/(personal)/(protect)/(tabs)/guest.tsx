...
const Guest = () => {
  ...
  const fetchEventAndGuests = async () => {
    ...
    try {
      ...
      const eventIdToLoad = selectedEventId || userEvents[0]._id;
      setSelectedEventId(eventIdToLoad);
      const res = await getInvitesByEvent(eventIdToLoad);
  ...
  const eventOptions = events.map((event) => ({
    key: event._id,
    value: `${event.location || "Unknown"} - ${new Date(
      event.date
    ).toLocaleDateString()}`,
  }));
        ...
        <SelectList
          setSelected={(val: string) => {
            ...
            (async () => {
              setLoading(true);
              try {
                const res = await getInvitesByEvent(val);
                setGuests(res.invites);
                setFilteredGuests(res.invites);
              } catch (err) {
                console.log("Error fetching guests for event:", err);
              } finally {
                setLoading(false);
              }
            })();
...
const styles = StyleSheet.create({
  ...
  guestName: { fontWeight: "bold", fontSize: 16, color: colors.text },
  guestEmail: { color: colors.placeholder },
